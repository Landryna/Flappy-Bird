...
void LCD_DrawImage(uint16_t x, uint16_t y, uint16_t w, uint16_t h, const uint16_t* data) {
	if (x > sLCD_DIS.LCD_Dis_Column || y > sLCD_DIS.LCD_Dis_Page || (x + w - 1) > sLCD_DIS.LCD_Dis_Column || (y + h - 1) > sLCD_DIS.LCD_Dis_Page) {
		return;
	}
	uint32_t i;
	...
	LCD_DC_1;
	LCD_CS_0;
	HAL_SPI_Transmit(&hspi1, (uint8_t*)data, sizeof(uint16_t)*w*h, HAL_MAX_DELAY);
	LCD_CS_1;
}
